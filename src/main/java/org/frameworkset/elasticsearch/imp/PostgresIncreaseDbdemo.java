 ...
 * @version 1.0
 */
public class PostgresIncreaseDbdemo {
	private static Logger logger = LoggerFactory.getLogger(PostgresIncreaseDbdemo.class);
	public static void main(String args[]){
		PostgresIncreaseDbdemo dbdemo = new PostgresIncreaseDbdemo();
...
//		importBuilder.setSql("select * from td_sm_log where LOG_OPERTIME > #[LOG_OPERTIME]");
		dbInputConfig.setSql("select * from batchtest1 where optime > #[optime]");
...
//		//设置任务执行拦截器结束，可以添加多个
        //增量配置开始
		importBuilder.setStatusDbname("postgres");//设置增量状态数据源名称
        importBuilder.setLastValueColumn("optime");//手动指定数字增量查询字段，默认采用上面设置的sql语句中的增量变量名称作为增量查询字段的名称，指定以后就用指定的字段
